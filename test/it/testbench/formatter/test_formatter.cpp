...
int main(int argc, char *argv[]){
    ...
    // Format for wrong file type and check if exception is handled correctly
    PRINT(">> Format for wrong file extension");
    fres->ext = "abc";
    try {
        ReturnCode retCode = fmtManager.formatForFile(report);
        PRINT(">>>> Report was formatted the same!!!");
        PRINT(fres->content);
    } catch (exception &ex) {
        PRINT("formatForFile with wrong file extension threw this exception");
    }
    ...
    // 5: format for XML file
    PRINT(">> Format for XML file");
    fres->ext = "xml";
    try {
        ReturnCode retCode = fmtManager.formatForFile(report);
        PRINT(fres->content);
    } catch (exception &ex) {
        PRINT("formatForFile in XML failed!!!");
    }
    return 0;
}
